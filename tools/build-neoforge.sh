...
# ── Artifact ──────────────────────────────────────────────────────────────────
NEW_JAR="${PROJECT_DIR}/build/libs/cfx-compat-epicfight-${label}-mc${mc_ver}-neoforge.jar"
if [[ ! -f "${NEW_JAR}" ]]; then
...
if [[ -z "${MC_INSTANCE}" ]]; then
    ...
    if [[ -d "${MODS_DIR}" ]]; then
        rm -f "${MODS_DIR}"/cfx-compat-epicfight-*-mc*-neoforge.jar
        echo "  removed old cfx-compat-epicfight NeoForge jars"
    else
